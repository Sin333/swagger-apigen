...
const renderObjectDecl = (
    ...
    if (config.useInterface && body.startsWith('{')) {
...
const renderEnumDecl = (type: Extract<IrType, { kind: 'enum' }>, config: ApigenConfig): string => {
    const name = canonicalTypeName(type.name, config);
...
const renderUnionDecl = (
    ...
    const parts = type.variants.map((v) => {
        ...
        return discriminatorExpr !== undefined
            ? `(${base} & { ${key}: ${discriminatorExpr} })`
            : base;
    });
